...
public class BuildTextParameterYAML extends BuildTextYAMLBaseImpl implements BuildTextParameter, Dumpable {

    private String name;

    protected Object node;

    public BuildTextParameterYAML (CallContext context, Object node, BuildTextYAMLMetainfo meta, String source_location_info) {
        super(context, node, meta, source_location_info);

        this.node = node;
        
        this.name = (node instanceof BuildTextYAMLDatainfo ? ((BuildTextYAMLDatainfo) node).getParameter() : null);
        String optname = meta.getOptionalParameter();
