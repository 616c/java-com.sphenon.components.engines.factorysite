...
abstract public class ScaffoldBaseFactory
  extends ScaffoldGeneric_BaseImpl
{
    static final public Class _class = ScaffoldBaseFactory.class;
    ...
    protected String cached_factory_var;
    protected boolean already_visited;
    protected boolean creation_done;
    ...
    abstract protected Object getFactoryInstance(CallContext context);

    abstract protected Object precreateInstance(CallContext context);

    abstract protected Object createInstance(CallContext context);

    abstract protected String getScaffoldId(CallContext context);

    abstract protected boolean canPrecreate(CallContext context);
    ...
    public Object getValueAsObject (CallContext call_context) throws DataSourceUnavailable, IgnoreErraneousDataSource {
        ...
        try {
            if ((runtimestep_level & RuntimeStepLevel.OBSERVATION_CHECKPOINT) != 0) { runtime_step = RuntimeStep.create(context, RuntimeStepLevel.OBSERVATION_CHECKPOINT, _class, "Building subaggregate at '%(info)' using factory '%(factory)'", "info", source_location_info, "factory", factoryclass.getName()); }

            boolean first_invocation = true;

            if ((notification_level & Notifier.DIAGNOSTICS) != 0) { NotificationContext.sendDiagnostics(context, "'%(scaffoldid)', getValueAsObject (%(info)), cache '%(cache)', visited '%(visited)', created '%(created)', current pass '%(currentpass)', scaffold pass '%(scaffoldpass)'...", "scaffoldid", this.getScaffoldId(context), "cache", this.cached_result, "visited", this.already_visited, "created", this.creation_done, "currentpass", this.factory_site.getCurrentPass(context), "scaffoldpass", this.pass, "info", this.getSourceLocationInfo(context)); }
            ...
            if (this.already_visited || just_precreate) {
                ...
                if (this.canPrecreate(context) == false) {
                    DataSourceUnavailable.createAndThrow(context, FactorySiteStringPool.get(context, "0.8.7" /* Object of type '%(type)' cannot be delivered, object aggregate contains a cyclic reference and factory does not provide a 'precreate' method */), "type", this.type.getName(context));
                    throw (DataSourceUnavailable) null; // compiler insists
                }
                
                Object result = precreateInstance(context);
                ...
                if ((notification_level & Notifier.DIAGNOSTICS) != 0) { NotificationContext.sendDiagnostics(context, "'%(scaffoldid)', getValueAsObject (%(info)) - done, result (precreated): '%(result)'", "scaffoldid", this.getScaffoldId(context), "info", this.getSourceLocationInfo(context), "result", result); }
                ...
                Object result = createInstance(context);
