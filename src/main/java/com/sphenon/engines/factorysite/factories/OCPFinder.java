                        ...
                        if (    ocp_variant != null
                            ...
                            for (String spe : spa) {
                                for (String ext : mea) {
                                    ...
                                    if ((this.notification_level & Notifier.VERBOSE) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "Factory_Aggregate[%(oid)].create(): trying: '%(fullpath)' - '%({\"not found\",\"found\"}[found])'", "oid", this, "fullpath", fullpath, "found", t.o(found ? 1 : 0)); }
                                    if (found) {
