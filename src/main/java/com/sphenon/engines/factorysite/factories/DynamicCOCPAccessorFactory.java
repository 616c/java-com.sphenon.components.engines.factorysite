...
public class DynamicCOCPAccessorFactory extends Class_DynamicClass<COCPAccessorFactory> {
    ...
    static public Config config = Configuration_DynamicCOCPAccessorFactory.get(RootContext.getInitialisationContext());

    // Construction -------------------------------------------------------------------

    public DynamicCOCPAccessorFactory(CallContext context, String full_class_name, FactorySite factory_site) {
        super(context, full_class_name, COCPAccessorFactory.class);
    ...
    protected boolean doGeneration(CallContext context) {
        return DynamicCOCPAccessorFactory.config.getDoGeneration(context);
    }

    protected boolean doCompilation(CallContext context) {
        return DynamicCOCPAccessorFactory.config.getDoCompilation(context);
    }

    protected boolean tryToLoadAsResource(CallContext context) {
        return DynamicCOCPAccessorFactory.config.getTryToLoadAsResource(context);
    }

    protected boolean useExistingResourceUnconditionally(CallContext context) {
        return DynamicCOCPAccessorFactory.config.getUseExistingResourceUnconditionally(context);
    }
