...
public class DynamicCOCPAccessorRetriever extends Class_DynamicClass<COCPAccessorRetriever> {
    ...
    static public Config config = Configuration_DynamicCOCPAccessorRetriever.get(RootContext.getInitialisationContext());

    // Construction -------------------------------------------------------------------

    public DynamicCOCPAccessorRetriever(CallContext context, String full_class_name, FactorySite factory_site) {
        super(context, full_class_name, COCPAccessorRetriever.class);
    ...
    protected boolean doGeneration(CallContext context) {
        return DynamicCOCPAccessorRetriever.config.getDoGeneration(context);
    }

    protected boolean doCompilation(CallContext context) {
        return DynamicCOCPAccessorRetriever.config.getDoCompilation(context);
    }

    protected boolean tryToLoadAsResource(CallContext context) {
        return DynamicCOCPAccessorRetriever.config.getTryToLoadAsResource(context);
    }

    protected boolean useExistingResourceUnconditionally(CallContext context) {
        return DynamicCOCPAccessorRetriever.config.getUseExistingResourceUnconditionally(context);
    }
