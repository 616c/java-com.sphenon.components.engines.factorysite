...
public class BuildTextRefByIdXML extends BuildTextXMLBaseImpl implements BuildTextRefById, Dumpable {

    protected String id_ref;

    public BuildTextRefByIdXML (CallContext context, Element node, String source_location_info) {
        super(context, node, source_location_info);
        this.id_ref = node.getAttribute(BuildTextKeywords.IDREF);
        String optidref = node.getAttribute(BuildTextKeywords.OPTIONALIDREF);
