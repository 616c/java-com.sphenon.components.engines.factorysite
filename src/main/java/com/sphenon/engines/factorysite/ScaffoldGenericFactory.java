...
public class ScaffoldGenericFactory
    extends ScaffoldBaseFactory
    implements ScaffoldGenericCOCPEnabled
{
    static final public Class _class = ScaffoldGenericFactory.class;
    ...
    protected SpecificScaffoldFactory_Factory scaffold_factory;
    ...
    public ScaffoldGenericFactory (CallContext context, SpecificScaffoldFactory_Factory scaffold_factory, Type type, Class factoryclass, boolean allow_dynamic_type_check, Method create_method, Method precreate_method, Method set_parameters_at_once, Method set_component_type_method, Type component_type, java.util.Hashtable par_entries, Method new_instance_method, Constructor constructor, boolean cons_context_par, Vector_ScaffoldParameter_long_ parameters, MatchResult match_result, FactorySiteListener listener, boolean is_singleton, boolean have_dynamic_parameters, FactorySite factory_site, String oid, int pass, Vector<String[]> pre_conditions, Vector<String[]> post_conditions, Vector<String[]> pre_build_scripts, Vector<String[]> post_build_scripts, String source_location_info, String problem_monitor_oid)  {
        ...
        this.precreate_method = precreate_method;
    ...
    protected boolean canPrecreate(CallContext context) {
        return (this.precreate_method == null ? false : true);
    }

    protected Object getFactoryInstance(CallContext context) {
        if (this.factoryinstance == null && Modifier.isStatic(this.create_method.getModifiers()) == false) {
            if (this.new_instance_method != null) {
                this.factoryinstance = ReflectionUtilities.invoke(context, new_instance_method, null, context);
            } else {
            ...
            if (this.set_component_type_method != null && this.component_type != null) {
                ReflectionUtilities.invoke(context, this.set_component_type_method, factoryinstance, context, this.component_type);
            }
        }
    ...
    protected Object precreateInstance(CallContext context) {
        Object result = ReflectionUtilities.invoke(context, precreate_method, this.getFactoryInstance(context), context);
        return result;
    }

    protected Object createInstance(CallContext context) {
        Object result = ReflectionUtilities.invoke(context, create_method, factoryinstance, context);
        return result;
    }

    protected String getScaffoldId(CallContext context) {
        return "Scaffold['" + (this.getType(context) == null ? "(null)" : this.getType(context).getName(context)) + "'/GenericFactory '" + this.factoryclass.getName() + "']";
    }

    public void setParameters(CallContext context) throws DataSourceUnavailable {
        if (this.factoryinstance == null && Modifier.isStatic(this.create_method.getModifiers()) == true) {
            return;
        }
    ...
    public void printCOCPCode2(CallContext context, StringWriter sw, Vector<Integer> dr, FactorySiteTextBased.Coder coder, String scaffold_index) {
        ...
        dr.add(COCPIndices.COCPScaffold_Factory);

        dr.add((type == null ? ((Integer) 0) : coder.writeText(context, type.getId(context))));
        dr.add(coder.writeCode(context, this.factoryclass.getName() + ".class"));
        ...
        dr.add(coder.writeBoolean(context, this.precreate_method != null));
        dr.add(coder.writeText(context, "Scaffold['" + (this.getType(context) == null ? "(null)" : this.getType(context).getName(context)) + "'/GenericFactory '" + this.factoryclass.getName() + "']"));
        dr.add(coder.writeBoolean(context, Modifier.isStatic(this.create_method.getModifiers())));
