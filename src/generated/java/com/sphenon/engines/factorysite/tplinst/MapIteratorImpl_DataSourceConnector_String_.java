...
public class MapIteratorImpl_DataSourceConnector_String_
    implements IteratorItemIndex_DataSourceConnector_String_
{
    private Map_DataSourceConnector_String_ map_t;
    private java.util.Hashtable map;
    private java.util.Set entry_set;
    private java.util.Iterator iterator;
    private java.util.Map.Entry me;

    public MapIteratorImpl_DataSourceConnector_String_ (CallContext context, java.util.Hashtable map, Map_DataSourceConnector_String_ map_t) {
        this.map_t = map_t;
        this.map = map;
    ...
    public DataSourceConnector getCurrent (CallContext context) throws DoesNotExist {
        if ( ! canGetCurrent(context)) { DoesNotExist.createAndThrow(context); }
        return (DataSourceConnector) me.getValue();
    }

    public DataSourceConnector tryGetCurrent (CallContext context) {
        if ( ! canGetCurrent(context)) { return null; }
        return (DataSourceConnector) me.getValue();
    }
    ...
    public Reference_DataSourceConnector_ getReferenceToCurrent (CallContext context) throws DoesNotExist {
        return map_t.getReference(context, this.getCurrentIndex(context));
    }

    public Reference_DataSourceConnector_ tryGetReferenceToCurrent (CallContext context) {
        if ( ! canGetCurrent(context)) { return null; }
        return map_t.tryGetReference(context, this.tryGetCurrentIndex(context));
    }

    public MapIteratorImpl_DataSourceConnector_String_ clone(CallContext context) {
